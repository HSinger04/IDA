...
<!--TODO Marie würde hier lieber J[:alpha:*] als RegEx verwenden -->
```{r}
...
<!-- TODO: I am not sure what exactly they expect us to do here-->
```{r, eval = F}
exp_data <- tibble(
...
weird_RTs %>%
  ...
  log(.) %>% # take the log
  mean(.) %>% # take the mean
  round(., digits = 3) # round to 3 significant digits
```
