...
```{r}
passed <- c(Jax = T, Jamie = F, Geralt = T) 
```
...
```{r}
contains_J <- str_detect(names(passed), "^J")
passed[contains_J]
```
...
S02           B             03
```
<!-- TODO: check if color of values is same as from solution --> 
```{r}
tribble(
     ~participant,   ~condition,     ~response,
     "S01",           "A",             32,
...
<!-- TODO: participants and conditions could still get shuffled-->
```{r, eval = F}
exp_data <- tibble(
  ...
  trial       = 1:100,
  response    = runif(100) * 100 %>% round(., digits = 2)
)
```
    ...
    if(length(input) > 1) {
      return tibble(
          indices = seq(2, length(input), by=2) %>% # All even indices
            input[.] %>% # look at input values for those indices
            ifelse(is.nan(.) or is.na(.), F, T) %>% # convert into boolean
            seq(2, length(input), by=2)[.] # only keep those with value T
        )  
    }
