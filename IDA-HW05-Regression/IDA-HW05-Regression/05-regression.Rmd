...
d_cleaned <- d_cleaned %>% 
  ...
  # keep only the main trials which have a correct corresponding background check: Is not included in the task, not sure if we should do this
  filter((condition == "Condition 0" | condition == "Condition 1" | condition == "Condition 6" | condition == "Condition 9" | condition == "Condition 10") & background_correct == TRUE)
    
nrow(IDA_data_KoF) - nrow(d_cleaned)
...
IDA_data_KoF_plot <- as.data.frame(t(IDA_data_KoF_plot))
IDA_data_KoF_plot

IDA_data_KoF_plot = brm(
  formula = response ~ condition,
  family = bernoulli(link = "logit"),
  data = aida::data_KoF_cleaned,
  prior = prior(student_t(1, 0, 2), class = 'b'),
  sample_prior = 'yes',
  iter = 20000
)

summary(IDA_data_KoF_plot)$fixed[,c("l-95% CI", "Estimate", "u-95% CI")]
```
