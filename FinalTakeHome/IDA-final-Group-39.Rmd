...
**Solution**
  ...
  > "Strategy": The participants showed preference for CA strategy in the easily-verbalizable condition, while they showed no preference for either strategy in the non-easily-verbalizable conditoin, so evidence in favor of the hypothesis.
...
Specifically, the independent variable, verbalizability of features, is divided into easily-verbalizable features and not-easily-verbalizable features and provided as a stimulus. Therefore, it is the categorical factor, and unordered. This stimulus sets as a between-subject independent variable.
...
**Solution:**
The additional hypothesis tested for in the analysis of strategy model fit is whether model type (CA vs FR) has an influence on the model fit (scores), so whether there is a general preference for one strategy (in this case CA), without the influence of condition.
...
Because the choice of strategy is binary, we assume a flat Beta as prior. 
$$
\begin{align*}
  P(\theta) & = Beta(1,1) \\
  P(\theta \mid N, k) & = \text{Beta}(\theta, 1+k, 1 + N - k) 
\end{align*}
$$
...
$$
\begin{align*}
  P(\theta \mid 680, 456)& = \text{Beta}(\theta, 1+456, 1+680-456) \\
  P(\theta \mid 680, 456)& = \text{Beta}(\theta, 457, 225)
\end{align*}
$$
...
```
The mean estimate for the bias towards use of strategy CA is positive, both bounds of the credible interval are bigger than 0, so the data provides evidence in favor of the hypothesis that across all trials, there is a preference for adopting the CA strategy.