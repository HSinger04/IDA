...
Answer:
Z will become wider, that is, its standard deviation will be 2, instead of 1,001. 
...
```{r}
### TODO: plotting for irvin_hall does not work :( sampling seems to be working bc x-axis is changed
plot_irwin_hall <- function(n_samples) {
tibble(
  uni = runif(n_samples, 0, 1),
  irvin_hall = sum(uni)
  ) %>% 
  ggplot(aes(x = irvin_hall)) +
  geom_density()
}
...
Asnwer:
Control: kc ~ Beta(Thetac | 1+126, 1+22000-126) = Beta(Thetac | 163, 21839)
Test: kt ~ Beta(Thetat | 1+8, 1+22000-8) = Beta(Thetat | 9, 21993)
...
Answer: 
Given this model and the provided data, we can assume with a certainty of 95% that the probability of infection by Covid19 without the vaccine should be between 0,0063% and 0,0086% with an expected value of 0,0074% (control group). With vaccine, the probability of infection should be between 0,0002% and 0,0007% with an expected value of 0,0004% (test group). 
...
TODO: transpose numbers somehow so they fit ?
