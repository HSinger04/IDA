...
# 2 I suppose they are asking for total number of subjects
data_BLJM %>% 
  group_by(submission_id) %>% #look at column of subject ids
  summarise() %>% #groups duplicates together
  nrow(.)  #counts number of rows 

# 3 mean of variable timeSpent
data_BLJM %>%
  summarise(
    timeSpent_mean = mean(timeSpent)
  )

# 4 mean of variable age
data_BLJM %>%
  summarise(
...
data_BLJM_processed <- 
  data_BLJM %>% 
  # creating column 'condition' by building entries from the first letter of option1 and option2 respectively, for each subject 
  mutate(
    condition = str_c(str_sub(option2, 1, 1), str_sub(option1, 1, 1))
  ) %>% 
  # only keep these columns in variable data_BLJM
...
data_BLJM_conditions_select <- data_BLJM_processed %>% 
   pivot_wider(names_from =condition, values_from = response) %>% # 
   select(-BM) %>%  
   dplyr::count(JM, LB)

data_BLJM_conditions_select
```
...
data_BLJM_conditions_select %>%
  ggplot(aes(
    ...
    # ... and y axis
    y = n,
    # use different colors for Jazz and Metal
    color = JM,
    # use different shapes for Jazz and Metal
    shape = JM,
    # we want to create points / lines for Jazz and Metal separately, not together
    group = JM
   )
  ) + 
  # plot the points with size 3
  geom_point(size = 3) +
  # create respective lines for Jazz and Metal
  geom_line() +
  # add labels describing plot
  labs(
    title = "Counts of responses of each music+subject pair", 
    x = "",
    y = ""
  )
```
...
People who chose Logic chose more often Jazz than Metal.
People who chose Metal chose more often Biology than Logic.
As such, the data provides counter-evidence to the lecturer's conjecture.
...
As mentioned in the web book, the mode "is the unique value that occurred most frequently in the data". Considering that we are measuring weights on a yoctogram level, even with data of ten thousand seagulls, it's highly likely that no weight measurement will occur twice (in which case, there would be no mode) or that maybe a very small number of (like max. 10) seagulls have the same weight (let's call that weight x) up to the yoctogram level. <!-- TODO: Talk about why the mode is insufficient due to those reasons mentioned below  -->(
In the latter case, the mode is not a good measure of tendency since while the weight x would be the most frequent one, it is likely that just by random chance, x happened to be the mode in our specific data set, while it could have just as well been a (completely) different value if we sampled another 10000 seagull weights.
...
```{r}
fr_and_le <- data_WHO %>% 
  # select only FertilityRate and LifeExpectancy
  select(FertilityRate, LifeExpectancy) %>%
...
There appears to be a negative correlation between FertilityRate and LifeExpectancy. That is, with larger FertilityRate values, LifeExpectanty values become lower and vice versa.   
However, this does not imply that the two are causally related.
...
correlation_WHO <- data_WHO %>% 
  ...
  filter(!(is.na(FertilityRate) | is.na(LifeExpectancy))) %>%
  group_by(Region) %>%
  # add column for correlations
  mutate(correlation = cor(FertilityRate, LifeExpectancy)) %>%
  # remove duplicate region rows
...
data_WHO %>% 
  ggplot(
    #map "FertilityRate" onto x-axis
    mapping = aes(x = FertilityRate, 
    #map "LifeExpectancy" onto y-axis             
                  y = LifeExpectancy))+
    #add a scatter plot
  geom_point()+
...
```{r}
facet <- data_WHO %>% 
  ggplot(mapping = aes(x = FertilityRate, y = LifeExpectancy))+
  geom_point()+
  geom_smooth(method = "lm", se = FALSE)+
labs(x = 'Fertility rate', y = 'Life expectancy')+
  #arrange scatter plots one for each level of 'Region'
  facet_wrap(~Region, 
  #make each facet have its own range of values(make them free in one dimension)
             scales = "free")
facet
```
...
```{r}
correlation_WHO <- ungroup(correlation_WHO)
...
facet+
  geom_text(data = correlation_WHO, aes(x = x, y = y, label = correlation))
```
